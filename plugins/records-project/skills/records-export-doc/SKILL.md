...
4. **Strip the wikilinks first.** `[[Master Summary]]` and `[[01 Master/Master Summary|Master Summary]]` won't resolve inside a Doc — replace each with its plain label (`Master Summary`) rather than leaving brackets or a broken reference. A Doc for a doctor should read as prose.
